...
mat1 = np.zeros((128,128))
mat2 = np.zeros((128,128,128))
my_notes = None
for j in range(8):
  orig = bundle[j].parse()
...
# initialize and fill in frequency matrix
  ...
  mat1[my_notes[0].pitch.midi][my_notes[1].pitch.midi] += 1
  for i in range (2, len(my_notes)):
    mat1[my_notes[i-1].pitch.midi][my_notes[i].pitch.midi] += 1
    mat2[my_notes[i-2].pitch.midi][my_notes[i-1].pitch.midi][my_notes[i].pitch.midi] += 1

#print(mat)

# set first note in generated song to first note in original song
song1 = [my_notes[0].pitch.midi] 
song2 = [my_notes[0].pitch.midi, my_notes[1].pitch.midi]
